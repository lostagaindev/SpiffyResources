...
public class SpiffyCalculator_old {
	...
	public static float AdvanceCalculation(String input){
		...
		while(!isBasicCalculation(input)){
			...
			//	input=input.replaceAll(replacethis, ""+BasicCalculation(input.substring(posoflastopp, posofnextopp)));
				...
				//Position of opperator
				
				               //gets the position in the string of the "-" operator
				               //the position is called the Index and starts counting from 0
				int posofopp = input.indexOf("-");
				
		        int posofnextopp = smallestNonZero(
		                   //we look for the index (position) within the string of the next operators
		        		   //we then pick the "smallest non zero" position, which would be the next operator after the "-"
		        		   input.indexOf("+",posofopp+1),
		        		  	input.indexOf("-",posofopp+1),
		        		  	input.indexOf("/",posofopp+1),
		        		  	input.indexOf("*",posofopp+1)
